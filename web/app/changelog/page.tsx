...
const ENTRIES: Entry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "Emerging player detection is a real, documented formula (docs/analytics/emerging-player-methodology.md, linked from the public Methodology page), not vibes. It computes a composite score from four signals — trend magnitude (average percentile improvement across tracked metrics over a 5-snapshot window, 45%), trend consistency (fraction of metrics with sustained upward movement, 30%), age weight (sigmoid centred at 24, 15%), and sample weight (minutes/qualification threshold, capped at 1.0, 10%) — with a minimum 0.50 composite threshold for inclusion. It runs as a step in the existing weekly refresh orchestration (after publish, like watch detection), is idempotent, and excludes unqualified players regardless of trend strength.",
      "A new /leagues index page lists every covered league organised by tier with team counts and coverage badges, linked from the primary navigation. The league hub page reuses the existing sub-navigation pattern (Overview / Per-90 stats / Statlas Index) and integrates Follow (Phase 10) and Add-to-Shortlist (Phase 7) actions on player rows throughout.",
      "8 new e2e tests (league index listing, hub page content + leaderboard count, honest standings note, methodology link, player profile navigation, header nav link, axe on both pages), 7 new unit tests (emerging-score computation: strong trend above threshold, flat below, unqualified excluded, idempotent rerun, younger > older, plus league hub aggregation tests) bringing the total to 296.",
    ],
  },
  {
    date: "2026-08-18",
