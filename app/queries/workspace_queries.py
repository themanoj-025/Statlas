...
def _owned_shortlist(db: Session, user_id: int, shortlist_id: int) -> Shortlist:
    from app.models import Shortlist

    sl = (
        db.query(Shortlist)
        .filter(Shortlist.id == shortlist_id, Shortlist.user_id == user_id)
        .first()
    )
    if sl is None:
    ...
    entry = (
        ...
        .filter(
            ...
            ShortlistEntry.shortlist_id.in_(
                db.query(ShortlistEntry.shortlist_id)
                .filter(ShortlistEntry.id == entry_id)
                .subquery()
            ),
        )
        .first()
