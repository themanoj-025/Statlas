...
class TransfermarktSource(MarketDataSource):
    ...
    def fetch_squad_players(
        ...
        for club in clubs:
            try:
                club_url = f"{TRANSFERMARKT_BASE}{club['squad_url']}"
                logger.info(
                ...
                logger.warning("  Failed to fetch squad for %s: %s", club["name"], exc)

        logger.info("Total players scraped from %s: %d", league_slug, len(all_players))
        return all_players

    def _extract_clubs_from_overview(self, soup: BeautifulSoup) -> list[dict[str, Any]]:
    ...
    def _parse_player_profile(self, soup: BeautifulSoup) -> dict[str, Any]:
        ...
        name_el = soup.select_one(
            "h1.data-header__headline, .data-header__headline-wrapper, h1.tw-text-2xl"
        )
        if name_el:
    ...
    def fetch_valuations(
        ...
        for pid, name in zip(player_ids, names):
            try:
                ...
                if slug:
                    profile_url = f"{TRANSFERMARKT_BASE}/{slug}/profil/spieler/{pid}"
    ...
    def fetch_market_value_history(self, player_id: int) -> list[dict[str, Any]]:
    ...
    def fetch_transfers(self, since: datetime) -> list[TransferRecord]:
        ...
        for slug in self.tiers["leagues"]:
            try:
                url = self._league_transfers_url(
                    slug, f"{since.year}-{str(since.year + 1)[-2:]}"
                )
                soup = self._soup(url)
    ...
    def _parse_league_transfers(
        ...
        transfer_sections = soup.select("div.box table.items tbody")
    ...
    def _parse_transfer_row(
        ...
        transfer_date = (
            _parse_date(date_cell.get_text(strip=True)) if date_cell else None
        )
            ...
            try:
                ...
                if slug:
                    ...
                    if slug:
                        ...
                        logger.debug(
                            "No slug for player %s, skipping contract fetch", pid
                        )
                        continue
                soup = self._soup(url)
